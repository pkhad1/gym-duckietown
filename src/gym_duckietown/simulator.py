...
class Simulator(gym.Env):
    ...
    def _init_vlists(self):
        ...
        # half_size = self.3176.road_tile_size / 2
    ...
    def update_physics(self, action, delta_time: float = None):
        ...
        self.wheelVels = action * self.robot_speed * 0.5
    ...
    def compute_reward(self, pos, angle, speed):
        ...
        try:
            ...
            # Compute the reward
            reward = +10.0 * speed * lp.dot_dir + -10 * np.abs(lp.dist) + +400 * col_penalty
        return reward

    def step(self, action: np.ndarray):
        action = np.clip(action, -1, 1)*0.5
